...
import { useReportMutation, type ReportPayload } from './useReportMutation'
...
describe('useReportMutation', () => {
  it('onMutate sets target pin badgeState to red in cache before fetch resolves (5.4)', async () => {
    ...
    const { result } = renderHook(() => useReportMutation(), { wrapper: makeWrapper(queryClient) })
    ...
    expect(badgeStateDuringFetch).toBe('red')
  })
  ...
  it('mutationFn throws and triggers onError when server returns non-OK HTTP status (5.9)', async () => {
    const queryClient = freshClient()
    vi.stubGlobal(
      'fetch',
