...
export default function OnboardingScreen() {
  ...
  const [heightFtPart, setHeightFtPart] = useState<number>(10)
  const [heightInPart, setHeightInPart] = useState<number>(0)
  const [validationError, setValidationError] = useState<string | null>(null)

  // M3: Separate string states allow free typing in number inputs; committed on blur
  const [lengthInput, setLengthInput] = useState<string>('25')
  const [heightFtInput, setHeightFtInput] = useState<string>('10')
  const [heightInInput, setHeightInInput] = useState<string>('0')
  ...
  function handleSelectRigType(rigType: RigType) {
    ...
    setLengthInput(String(defaults.lengthFt)) // M3: sync display string
    const { ft, inches } = fromDecimalFeet(defaults.heightFt)
    setHeightFtPart(ft)
    setHeightInPart(inches)
    setHeightFtInput(String(ft)) // M3: sync display string
    setHeightInInput(String(inches)) // M3: sync display string
  }

  function handleLengthChange(value: number) {
    const clamped = Math.max(LENGTH_MIN, Math.min(LENGTH_MAX, value))
    setLengthFt(clamped)
    setLengthInput(String(clamped)) // M3: keep display in sync after stepper
  }
  ...
  function handleHeightFtChange(value: number) {
    ...
    setHeightFtInput(String(clamped)) // M3: keep display in sync
  ...
  function handleHeightInChange(value: number) {
    ...
    setHeightInInput(String(clamped)) // M3: keep display in sync
  }
  ...
  return (
    <div className="min-h-screen bg-background text-foreground flex flex-col">
      ...
      <div className="px-6 pt-10 pb-6">
        <h1 className="text-2xl font-bold">Set up your rig</h1>
      ...
      <div className="flex-1 px-6 space-y-8 pb-36 overflow-y-auto">
        ...
        <section>
          ...
          {/* M4: radiogroup semantics — chips are mutually exclusive, not independent toggles */}
